...
async function loadStudios(filters = {}) {
  try {
    ...
    if (d.success) {
      ...
      const badge = document.getElementById('studiosCountBadge');
      if (badge) badge.textContent = d.data.length + ' espaces';
      // Si filtres actifs → réécrire la grille
      if (params) {
        renderStudioCards(d.data);
      } else {
        // Sans filtre : garder les cartes statiques avec photos
        // Juste attacher les IDs depuis la DB aux cartes existantes
        attachStudioEvents(d.data);
      }
    }
  } catch (e) { console.error('Erreur chargement studios', e); }
}

function attachStudioEvents(studios) {
  // Attacher les vrais IDs de la DB aux cartes statiques
  const cards = document.querySelectorAll('#studiosGrid .studio-card, .studios-featured-grid .studio-card-big');
  cards.forEach((card, i) => {
    if (studios[i]) {
      const id = studios[i].id;
      card.onclick = () => openStudioDetail(id);
      const btn = card.querySelector('.card-btn, button');
      if (btn) btn.onclick = (e) => { e.stopPropagation(); openStudioDetail(id); };
    }
  });
}
