...
// ── FIX LIENS NAVIGATION ──
function fixNavLinks() {
  // Explorer → scroll vers studios
  document.querySelectorAll('a[href="#"], nav ul a').forEach(a => {
    const text = a.textContent.trim().toLowerCase();
    if (text === 'explorer') {
      a.onclick = (e) => { e.preventDefault(); document.getElementById('studios')?.scrollIntoView({behavior:'smooth'}); };
    }
    if (text === 'comment ça marche') {
      a.onclick = (e) => { e.preventDefault(); document.getElementById('comment')?.scrollIntoView({behavior:'smooth'}); };
    }
    if (text === 'villes') {
      a.onclick = (e) => { e.preventDefault(); document.querySelector('section:has(.cities-scroll)')?.scrollIntoView({behavior:'smooth'}) || document.querySelectorAll('section')[3]?.scrollIntoView({behavior:'smooth'}); };
    }
    if (text === 'proposer un studio') {
      a.onclick = (e) => { e.preventDefault(); window.location.href='/proposer'; };
    }
  });
}

// ── MODAL PROPOSER UN STUDIO ──
function injectModalProposer() {
  if (document.getElementById('modalProposer')) return;
  const modal = document.createElement('div');
  modal.className = 'modal-overlay';
  modal.id = 'modalProposer';
  modal.innerHTML = `
    <div class="modal" style="position:relative;max-width:560px;">
      <button class="modal-close" onclick="closeModal('modalProposer')">✕</button>
      <h2>Proposer mon studio</h2>
      <p class="sub">Remplissez ce formulaire — nous vous recontactons sous 48h</p>

      <div class="form-row">
        <div class="form-group">
          <label>Votre nom</label>
          <input type="text" class="form-input" id="propNom" placeholder="Prénom Nom">
        </div>
        <div class="form-group">
          <label>Téléphone</label>
          <input type="tel" class="form-input" id="propTel" placeholder="+221 77 000 0000">
        </div>
      </div>
      <div class="form-group">
        <label>Email</label>
        <input type="email" class="form-input" id="propEmail" placeholder="votre@email.com">
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Nom du studio</label>
          <input type="text" class="form-input" id="propStudioNom" placeholder="Ex: Studio Lumière">
        </div>
        <div class="form-group">
          <label>Ville</label>
          <select class="form-input" id="propVille">
            <option>Dakar</option>
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          <label>Prix / heure (XOF)</label>
          <input type="number" class="form-input" id="propPrix" placeholder="Ex: 15000">
        </div>
      </div>
      <div class="form-group">
        <label>Description</label>
        <textarea class="form-input" id="propDescription" rows="3" placeholder="Décrivez votre espace, équipements disponibles..." style="resize:vertical;"></textarea>
      </div>
      <div class="form-error" id="propError"></div>
      <div class="form-success" id="propSuccess"></div>
      <button class="btn-full" id="propBtn" onclick="submitProposerStudio()">Envoyer ma demande 🚀</button>
    </div>`;
  modal.onclick = (e) => { if (e.target === modal) closeModal('modalProposer'); };
  document.body.appendChild(modal);
}

async function submitProposerStudio() {
  const nom         = document.getElementById('propNom').value.trim();
  const tel         = document.getElementById('propTel').value.trim();
  const email       = document.getElementById('propEmail').value.trim();
  const studioNom   = document.getElementById('propStudioNom').value.trim();
  const ville       = document.getElementById('propVille').value;
  const type        = document.getElementById('propType').value;
  const prix        = document.getElementById('propPrix').value;
  const description = document.getElementById('propDescription').value.trim();
  const btn         = document.getElementById('propBtn');
  const err         = document.getElementById('propError');
  const suc         = document.getElementById('propSuccess');

  if (!nom || !email || !tel || !studioNom || !prix) {
  ...
  try {
    const r = await fetch('/api/proposer-studio', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ nom, tel, email, studioNom, ville, type, prix, description })
    });
    const d = await r.json();

    if (d.success) {
      err.classList.remove('show');
      suc.textContent = '✅ Demande envoyée ! Nous vous recontactons sous 48h.';
      suc.classList.add('show');
      btn.style.display = 'none';
      showToast('success', 'Demande envoyée !', 'Notre équipe vous contactera sous 48h.');
    } else {
      err.textContent = d.message || 'Erreur lors de l\'envoi.';
      err.classList.add('show');
      btn.disabled = false;
...
function fixButtons() {
  ...
  document.querySelectorAll('.btn-white').forEach(btn => {
    btn.onclick = (e) => { e.preventDefault(); window.location.href='/proposer'; };
  });
